...
// ── Theme tokens ─────────────────────────────────────────────────────
// All visual differences live here — the single <form> below just binds these.

const wrapperClass = computed(() => {
  if (isEmergency.value)
    return 'rounded-2xl border border-rose-900/30 bg-slate-900/80 p-7 shadow-xl backdrop-blur-sm ring-1 ring-white/5'
  if (isGrooming.value)
...
const labelClass = computed(() =>
  isEmergency.value
    ...
    : 'block text-sm font-semibold mb-1 text-slate-600 dark:text-slate-300'
)
...
const buttonClass = computed(() => {
  if (isEmergency.value)
    return 'w-full py-3.5 rounded-lg font-black text-white text-[15px] bg-rose-600 hover:bg-rose-700 shadow-lg shadow-rose-900/40 transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.98]'
  if (isGrooming.value)
...
const notesLabel = computed(() =>
  (isEmergency.value || isVet.value) ? 'Symptoms / Notes' : 'Notes'
)
...
<template>
  <!-- ── EMERGENCY TRACKER (only rendered for emergency theme) ─────── -->
  <div v-if="isEmergency" class="w-full max-w-lg">
    <Transition name="slide-up">
      ...
      >
        <div class="mb-6 text-center">
          ...
          <h2 class="mt-2 text-xl font-black text-white">Help is on the way, {{ submittedPetName }}!</h2>
        ...
        <div class="mb-6 flex items-center gap-3 rounded-xl border border-emerald-900/30 bg-emerald-950/20 px-4 py-3">
          ...
          <span
            ...
            :class="currentStage >= 2 ? 'bg-emerald-500/10 text-emerald-400 ring-1 ring-emerald-500/20' : 'bg-slate-700/50 text-slate-500'"
          >
          ...
          <div v-for="(stage, i) in stages" :key="stage.label" class="flex items-start gap-4">
            <div class="flex flex-col items-center">
              <div
                ...
                :class="i < currentStage ? 'border-emerald-500 bg-emerald-500/10' : i === currentStage ? 'border-rose-500 bg-rose-500/10 ring-4 ring-rose-500/10' : 'border-slate-700 bg-slate-800/50'"
              >
              ...
              <div v-if="i < stages.length - 1" class="w-0.5 h-6 mt-1 transition-colors duration-700" :class="i < currentStage ? 'bg-emerald-500/40' : 'bg-slate-700/40'" />
            </div>
            <div class="pb-4 pt-1">
              <p class="text-[13px] font-bold transition-colors duration-300" :class="i <= currentStage ? 'text-white' : 'text-slate-600'">{{ stage.label }}</p>
              <p v-if="i === currentStage" class="mt-0.5 text-[11px] text-slate-400">{{ stage.detail }}</p>
              <p v-else-if="i < currentStage" class="mt-0.5 text-[11px] text-emerald-500/70">{{ stage.detail }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between rounded-xl border border-slate-700/50 bg-slate-800/40 px-4 py-3 mb-4">
          ...
          <a href="tel:+94112345678" class="text-[12px] font-black text-rose-400 transition hover:text-rose-300">📞 Call Hotline</a>
        </div>

        <button type="button" @click="cancelTracking"
  ...
  <!-- ── SINGLE SHARED FORM (all themes, hidden only during emergency tracking) ── -->
  <Transition name="slide-up">
    <div v-if="!tracking" :class="wrapperClass">
      ...
      <div v-if="isEmergency || isGrooming || isVet" class="mb-6 text-center">
        ...
        <h2 :class="headingClass">{{ heading || 'Book Service' }}</h2>
        <p class="mt-1 text-[12px] text-slate-400">{{ subheading }}</p>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          <label :class="labelClass">{{ notesLabel }}</label>
          <textarea v-model="form.notes" rows="3" :class="inputClass + ' resize-none'"
            :placeholder="notesPlaceholder ?? notesDefault" />
        </div>
