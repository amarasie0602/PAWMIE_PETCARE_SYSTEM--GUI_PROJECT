...
const reviewContent: Omit<Review, 'name' | 'location' | 'avatar'>[] = [
  { id: 'r-1', petName: 'Biscuit', petType: 'Golden Retriever', rating: 5, service: 'Vet Appointment', review: 'Booked a same-day check-up for Biscuit. The vet already had his notes - first visit where he did not hide under the chair.' },
...
const fallbackNames = ['Sarah Mitchell', 'James Okonkwo', 'Tom Harrington', 'Priya Nair', 'Daniel Fernandez', 'Anika Patel']
const fallbackLocations = ['New York, NY', 'Chicago, IL', 'Los Angeles, CA', 'Austin, TX', 'Miami, FL', 'Seattle, WA']
...
onMounted(async () => {
  try {
    ...
    reviews.value = reviewContent.map((review, i) => ({
      ...review,
      name: fallbackNames[i] ?? 'Pet Parent',
      location: fallbackLocations[i] ?? '',
  ...
  <section class="pb-14 pt-4">
    <div :ref="revealHeader.add" class="mb-8 text-center">
      ...
      <div class="mt-3 flex items-center justify-center gap-2">
        ...
        <span class="text-[12px] text-slate-400 dark:text-slate-500">&middot; 2,400+ families</span>
      </div>
    </div>
        ...
        <div class="mt-4 flex items-center gap-3 border-t border-slate-100 pt-4 dark:border-white/[0.06]">
          ...
          <div>
            ...
            <p class="text-[11px] text-slate-400 dark:text-slate-500">
              {{ review.petName }} &middot; {{ review.petType }}
            </p>
          </div>
        </div>
      </article>
