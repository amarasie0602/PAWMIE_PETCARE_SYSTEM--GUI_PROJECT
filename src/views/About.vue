  ...
  <template>
    <div class="min-h-screen -mt-20 pt-24 pb-20 bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-100 dark:from-slate-900 dark:via-slate-950 dark:to-indigo-950">
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div class="space-y-5">
            ...
            <div class="space-y-4">
              <article
                ...
                :key="value.id"
                class="rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-white/90 dark:bg-slate-900/90 p-4 shadow-sm"
              >
        ...
        <!-- Team -->
        <section>
          <h2 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white mb-6 text-center">
            The demo team behind Pawmie
          </h2>
          <div class="grid gap-6 sm:grid-cols-2">
            <article
              v-for="member in data.team"
              :key="member.id"
              class="rounded-2xl bg-white/90 dark:bg-slate-900/95 border border-slate-200/80 dark:border-slate-800/80 p-5 shadow-sm"
            >
              <h3 class="text-base font-semibold text-slate-900 dark:text-white">
                {{ member.name }}
              </h3>
              <p class="text-xs font-semibold uppercase tracking-[0.18em] text-indigo-500 dark:text-indigo-300 mt-1">
                {{ member.role }}
              </p>
